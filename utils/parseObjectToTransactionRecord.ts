...
function get_TransactionRecord_validity_warnings(original_record:FileDataRecord,transactionRecord:TransactionRecord):string[]{
    const warningMessages: string[] = []

    if (!isValidDateFns(transactionRecord.Date)){
        warningMessages.push(`Invalid Date: ${transactionRecord.Date} is not valid.`)
    }
    ...
    if (convertPenceToPounds(transactionRecord.Amount).toString() !== original_record.Amount.trim()){
...
function parseDataRecordToTransactionRecords(Record:FileDataRecord, line_index_number:number): TransactionRecord | null{
    ...
    const warnings = get_TransactionRecord_validity_warnings(Record, newTransactionRecord);
...
export function getParsedObjectsToTransactionRecords(
  dataRecords: FileDataRecord[],
    ...
    if (!isAllValid){
        logger.warn("The above warnings were found while parsing the records. Please fix the issues and try again.")
        process.exit(1);
    }
    
    logger.info(`Successfully parsed ${parsedDataRecords.length} valid records out of ${dataRecords.length} total records.`)
